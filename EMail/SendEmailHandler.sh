...
readonly SCRIPT_TNAME="SendEmailHandler"
...
readonly SCRIPT_EMAIL_CONFIG_FPATH="${ADDONS_SHARED_LIBS_DIR_PATH}/${SCRIPT_TNAME}.conf"

readonly CEM_LIB_BRANCH="develop"   ##**TBD "master" RELEASE**##
...
Script to send email notifications from the router.
...
To uninstall the script and its configuration file: 
   ...
   ${GRNct}./$scriptFileName ${CYANct}-uninstall${CLRct}


To view the current script configuration file:

   ${GRNct}./$scriptFileName ${CYANct}-viewconf${CLRct}
...
[The order of the secondary arguments after the FIRST is important]
   ...
   ${GRNct}./$scriptFileName ${CYANct}-test${CLRct}

   ${GRNct}./$scriptFileName ${CYANct}-test${CLRct} "SUBJECT_LINE"

   ${GRNct}./$scriptFileName ${CYANct}-test${CLRct} "SUBJECT_LINE" "EMAIL_BODY_TITLE_LINE"

   ${GRNct}./$scriptFileName ${CYANct}-test${CLRct} ${MGNTct}-From=${CLRct}"FROM_NAME" "SUBJECT_LINE"

   ${GRNct}./$scriptFileName ${CYANct}-test${CLRct} ${MGNTct}-From=${CLRct}"FROM_NAME" "SUBJECT_LINE" "EMAIL_BODY_TITLE_LINE"
   ...
   ${GRNct}./$scriptFileName ${CYANct}-test${CLRct} "TEST Email Subject"

   ${GRNct}./$scriptFileName ${CYANct}-test${CLRct} "TEST Email Subject" "TESTING My Email Notification"

   ${GRNct}./$scriptFileName ${CYANct}-test${CLRct} ${MGNTct}-From=${CLRct}"SenderID" "TEST Email Subject"
   
   ${GRNct}./$scriptFileName ${CYANct}-test${CLRct} ${MGNTct}-From=${CLRct}"SenderID" "TEST Email Subject" "TESTING My Email Notification"

EOF1

   cat <<EOF2 | xargs -0 echo -e

To send an email notification [the order of the secondary arguments after the FIRST is important]:

   ${GRNct}./$scriptFileName ${CYANct}-send${CLRct} "SUBJECT_LINE" "EMAIL_BODY_MSG_STRING"

   ${GRNct}./$scriptFileName ${CYANct}-send${CLRct} "SUBJECT_LINE" "EMAIL_BODY_MSG_STRING" "EMAIL_BODY_TITLE_LINE"

   ${GRNct}./$scriptFileName ${CYANct}-send${CLRct} "SUBJECT_LINE" ${MGNTct}-File=${CLRct}"EMAIL_BODY_MSG_FILE"

   ${GRNct}./$scriptFileName ${CYANct}-send${CLRct} "SUBJECT_LINE" ${MGNTct}-File=${CLRct}"EMAIL_BODY_MSG_FILE" "EMAIL_BODY_TITLE_LINE"

   ${GRNct}./$scriptFileName ${CYANct}-send${CLRct} ${MGNTct}-From=${CLRct}"FROM_NAME" "SUBJECT_LINE" "EMAIL_BODY_MSG_STRING"

   ${GRNct}./$scriptFileName ${CYANct}-send${CLRct} ${MGNTct}-From=${CLRct}"FROM_NAME" "SUBJECT_LINE" "EMAIL_BODY_MSG_STRING" "EMAIL_BODY_TITLE_LINE"

   ${GRNct}./$scriptFileName ${CYANct}-send${CLRct} ${MGNTct}-From=${CLRct}"FROM_NAME" "SUBJECT_LINE" ${MGNTct}-File=${CLRct}"EMAIL_BODY_MSG_FILE"

   ${GRNct}./$scriptFileName ${CYANct}-send${CLRct} ${MGNTct}-From=${CLRct}"FROM_NAME" "SUBJECT_LINE" ${MGNTct}-File=${CLRct}"EMAIL_BODY_MSG_FILE" "EMAIL_BODY_TITLE_LINE"
   ...
   ${GRNct}./$scriptFileName ${CYANct}-send${CLRct} "Email Subject Line" "My Simple Email Body Message String"

   ${GRNct}./$scriptFileName ${CYANct}-send${CLRct} ${MGNTct}-From=${CLRct}"SenderID" "Email Subject Line" "My Simple Email Body Message String"

   ${GRNct}./$scriptFileName ${CYANct}-send${CLRct} ${MGNTct}-From=${CLRct}"SenderID" "Email Subject Line" "My Simple Email Body Message String" "Email Body Title Line"
   ...
   ${GRNct}./$scriptFileName ${CYANct}-send${CLRct} "Email Subject Line" ${MGNTct}-File=${CLRct}"${TMP_DIR}/theEmailBody.txt" 

   ${GRNct}./$scriptFileName ${CYANct}-send${CLRct} ${MGNTct}-From=${CLRct}"SenderID" "Email Subject Line" ${MGNTct}-File=${CLRct}"${TMP_DIR}/theEmailBody.txt"

   ${GRNct}./$scriptFileName ${CYANct}-send${CLRct} ${MGNTct}-From=${CLRct}"SenderID" "Email Subject Line" ${MGNTct}-File=${CLRct}"${TMP_DIR}/theEmailBody.txt" "Email Body Title Line"
...
OPTIONAL argument modifiers following the FIRST parameter:
   ...
   ${GRNct}./$scriptFileName ${CYANct}-test -ptext${CLRct}

   ${GRNct}./$scriptFileName ${CYANct}-send -ptext${CLRct} "Email Subject Line" "Email Body Message String"

   ${GRNct}./$scriptFileName ${CYANct}-send -ptext -quiet${CLRct} ${MGNTct}-From=${CLRct}"SenderID" "Email Subject Line" "Email Body Message String" "Email Body Title Line"
...
See configuration file ${CYANct}${SCRIPT_EMAIL_CONFIG_FPATH}${CLRct} for global defaults.
...
_CreateConfigDefaultsFile_()
{
   ...
   } > "$SCRIPT_EMAIL_CONFIG_FPATH"
   chmod 644 "$SCRIPT_EMAIL_CONFIG_FPATH"
}
...
_DownloadScriptFile_()
{
   ...
   then
       ...
       theMsgStr="${REDct}**ERROR**${CLRct}: Unable to download the script file [$theDestFName] [${MGNTct}${statusSTRx}${CLRct}]"
   ...
   do
       ...
       then
           ...
           then
               ...
               _PrintMsg_ "The latest shared email library script [$CUSTOM_EMAIL_LIB_SCRIPT_FNAME] was ${actionStr2}.\n"
           fi
           retCode=0
           break
       fi
...
_CheckScriptVersionUpdate_()
{
   ...
   "$isVerboseMode" && \
   _PrintMsg_ "\nChecking for shared email script updates...\n"
   ...
   then
       rm -f "$theTmpFilePath"
       _PrintMsg_ "You have the latest email script version [${GRNct}${SCRIPT_VERSION}_${SCRIPT_VERSTAG}${CLRct}] installed.\n"
       return 0
   fi

   _PrintMsg_ "New shared email script version [${MGNTct}${dlVersionStr}_${dlVersTagStr}${CLRct}] is available.\n"
   mv -f "$theTmpFilePath" "$theScriptFPath"
   chmod 755 "$theScriptFPath"
   _PrintMsg_ "Shared email script has been updated to the latest version [${GRNct}${dlVersionStr}_${dlVersTagStr}${CLRct}].\n"
...
_SendEMail_()
{
   ...
   elif _ExcludeOptionalEmailArgs_ "$2"
   then
   ...
   if [ $# -gt 2 ] && [ -n "$3" ] && _ExcludeOptionalEmailArgs_ "$3"
...
if [ "$action" = "-uninstall" ]
then
    rm -f "$0" "$SCRIPT_EMAIL_CONFIG_FPATH"
    _PrintMsg_ "\nScript [$0] was uninstalled.\n\n"
    exit 0
fi

if [ ! -s "$SCRIPT_EMAIL_CONFIG_FPATH" ]
then _CreateConfigDefaultsFile_
fi
_DOStoUNIX_ "$SCRIPT_EMAIL_CONFIG_FPATH"

if [ "$action" = "-viewconf" ]
then
    printf "\n${GRNct}${SCRIPT_EMAIL_CONFIG_FPATH}${CLRct}\n"
    printf "----------------------------------------------\n"
    cat "$SCRIPT_EMAIL_CONFIG_FPATH"
    printf "----------------------------------------------\n"
    exit 0
fi
. "$SCRIPT_EMAIL_CONFIG_FPATH"
